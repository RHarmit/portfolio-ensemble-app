...
def compute_technicals(price_series):
    """
    Returns RSI, 50MA signal, MACD signal, and combined score.
    Each bullish signal = +1, bearish = -1, neutral = 0.
    Final: score >= 2 = Strong, score <= -2 = Weak, else Neutral.
    """
    ...
    if rsi_val < 30:
        score += 1      # oversold = potential bounce = bullish signal
        rsi_label = f"{rsi_val:.1f} (Oversold)"
    elif rsi_val > 70:
        score -= 1      # overbought = caution = bearish signal
        rsi_label = f"{rsi_val:.1f} (Overbought)"
    else:
        rsi_label = f"{rsi_val:.1f} (Neutral)"
    details["RSI"] = rsi_label
...
# ===== Output 4: Combined technical + sentiment =====
st.subheader("Technical and Sentiment Overview per stock")
st.write("Technical signal combines RSI, 50-day MA, and MACD. Sentiment scores latest news headlines.")
...
def color_signal(val):
    if val == "Strong" or val == "Positive":
        return "background-color: #d4edda; color: #155724"
    elif val == "Weak" or val == "Negative":
...
st.dataframe(
    overview_df.style.applymap(
        color_signal, subset=["Technical Signal", "News Sentiment"]
    )
)
