...
st.write("Enter your tickers and backtest period, then run the models to get suggested weights for next year.")

# --- 1. User inputs via Streamlit ---
...
start_year = col1.number_input("Start year", min_value=1980, max_value=date.today().year, value=2018, step=1)
end_year   = col2.number_input("End year",   min_value=start_year, max_value=date.today().year, value=date.today().year, step=1)

risk_free = st.number_input("Risk-free rate (annual, e.g. 0.02 for 2%)", value=0.02, step=0.01)
...
if st.button("Run optimization"):
    ...
    st.write(f"Fetching Adjusted Close data for {tickers} from {start_date} to {end_date}...")
    ...
    st.subheader("Benchmark: Equal-weight portfolio")
    df_eq, meta_eq = stats_frame("Equal-weight", eq_dict, eq_stats)
    st.dataframe(df_eq.style.format({"Weight %": "{:.2f}"}))
    st.json(meta_eq)
    ...
    st.subheader(f"Model 1: Constrained Max-Sharpe ({start_year}-{end_year})")
    df_ms, meta_ms = stats_frame("Max-Sharpe", w_ms, stats_ms)
    st.dataframe(df_ms.style.format({"Weight %": "{:.2f}"}))
    st.json(meta_ms)
    ...
    st.subheader(f"Model 2: Min-Volatility ({start_year}-{end_year})")
    df_mv, meta_mv = stats_frame("Min-Vol", w_mv, stats_mv)
    st.dataframe(df_mv.style.format({"Weight %": "{:.2f}"}))
    st.json(meta_mv)
    ...
    st.subheader("Model 3: Risk-Parity (inverse-vol approximation)")
    df_rp, meta_rp = stats_frame("Risk-Parity", w_rp, stats_rp)
    st.dataframe(df_rp.style.format({"Weight %": "{:.2f}"}))
    st.json(meta_rp)
    ...
    st.subheader("FINAL: Ensemble portfolio (average of Max-Sharpe, Min-Vol, Risk-Parity)")
    df_ens, meta_ens = stats_frame("Ensemble", w_ens, stats_ens)
    st.dataframe(df_ens.style.format({"Weight %": "{:.2f}"}))
    st.json(meta_ens)
