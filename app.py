...
# ===== Model 4: Momentum (12-1 month) =====
# 12-1 momentum: return from 12 months ago to 1 month ago (skip last month to avoid reversal)
# if not enough data, fall back to equal weight
try:
    # need at least 252 trading days
    if len(prices) >= 252:
        ret_12m  = prices.iloc[-252] / prices.iloc[0] - 1     # 12 month ago return
        ret_1m   = prices.iloc[-21]  / prices.iloc[0] - 1     # 1 month ago return
        momentum = ret_12m - ret_1m                            # 12-1 momentum score

        # only go long on positive momentum stocks
        ...
        w_mom = eq_weights.copy()
        st.warning("Not enough data for momentum signal. Using equal weight for Model 4.")
except Exception:
    w_mom = eq_weights.copy()
    st.warning("Momentum calculation failed. Using equal weight for Model 4.")
...
# ===== Seasonality: best calendar month per stock =====
...
# ===== Output 2: Individual model weights side by side =====
...
# ===== Output 4: Best calendar month per stock =====
...
# ===== Output 5: Full seasonality heatmap =====
